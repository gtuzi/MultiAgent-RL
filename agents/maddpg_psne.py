...
import numpy as np
from agents.ddpg_psne import DDPGPSNEAgent
import torch
...
class MADDPG_PSNE:
    def __init__(self, n_agents, partial_obs_size,
                 action_size, full_obs_size,
                 discount_factor=0.995,
                 tau=0.02,
                 logger = None):
        super(MADDPG_PSNE, self).__init__()
        ...
        self.maddpg_agent = [DDPGPSNEAgent(in_actor = partial_obs_size,out_actor = action_size,
                                           in_critic= full_obs_size + n_agents*action_size,
                                           lr_actor=1.0e-2,lr_critic=1.0e-2),
                             DDPGPSNEAgent(in_actor=partial_obs_size, out_actor=action_size,
                                           in_critic=full_obs_size + n_agents * action_size,
                                           lr_actor=1.0e-2, lr_critic=1.0e-2)]
    ...
    def act(self, obs_all_agents, use_perturbed_actor=True, noise_delta = 0.2):
        """get actions from all agents in the MADDPG object"""
        actions = []
        for agent, obs in zip(self.maddpg_agent, obs_all_agents):
            a = agent.act(obs, use_perturbed_actor, noise_delta=noise_delta)
            actions.append(a)
        return actions
    ...
    def perturb_the_actors(self, samples):
        all_agents_partial_obs = []
        for e in samples:
            all_agents_partial_obs.append(e[0])  # sample x actor x obs size

        all_agents_partial_obs = torch.tensor(all_agents_partial_obs, dtype=torch.float32)

        for ai, agent in enumerate(self.maddpg_agent):
            agent.perturb_the_actor(all_agents_partial_obs[:,ai,:])
    ...
    def learn(self, samples, agent_number):
        ...
        this_agent_partial_obs = torch.tensor(this_agent_partial_obs, dtype=torch.float32)
        all_agents_partial_obs = torch.tensor(all_agents_partial_obs, dtype=torch.float32)
        full_obs = torch.tensor(full_obs, dtype=torch.float32)
        this_agent_action = torch.tensor(this_agent_action, dtype=torch.float32)
        all_actors_actions = torch.tensor(all_actors_actions, dtype=torch.float32)
        reward = torch.tensor(reward, dtype=torch.float32)
        next_partial_obs = torch.tensor(next_partial_obs, dtype=torch.float32)
        next_partial_obs_all_actors = torch.tensor(next_partial_obs_all_actors, dtype=torch.float32)
